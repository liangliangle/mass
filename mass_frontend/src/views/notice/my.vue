  ...
  <section>
    <el-table v-loading="loading" :data="this.request.list" style="width: 100%">
      ...
      <el-table-column
        ...
        label="阅读状态">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.redundancy==0" type="warning">
            未读
          </el-tag>
          <el-tag v-if="scope.row.redundancy==1">已读</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        label="操作">
...
<script>
  import {getMy} from "@/service/noticeapi";
