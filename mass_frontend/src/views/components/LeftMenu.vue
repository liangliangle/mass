...
<template>
  <el-menu :collapse="collapsed"
    ...
    <template v-for="menu in menus">
      ...
      <!--判断是否有子菜单，有子菜单生成子菜单-->
      <el-submenu v-if="menu.children != null" :index="menu.url">
        <template slot="title">
          <i :class="menu.pcIcon"></i>
          <span>{{menu.name}}</span>
        </template>
        <template v-if="menu.children != null" v-for="menu2 in menu.children">
          <!--判断是否有子菜单，无子菜单则直接生成菜单项-->
          <el-menu-item v-if="menu2.children == null" :index="menu2.url">
            <i :class="menu2.pcIcon"></i>
            <span slot="title">{{menu2.name}}</span>
          </el-menu-item>
          <!--判断是否有子菜单，有子菜单生成子菜单-->
          <el-submenu v-if="menu2.children != null" :index="menu2.url">
